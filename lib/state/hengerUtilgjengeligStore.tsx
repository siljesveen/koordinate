...
type HengerUtilgjengeligStoreValue = {
  ...
  lagre: (item: HengerUtilgjengelig) => void;
  slett: (id: string) => void;
};

const STORAGE_KEY = "bemanning.hengerUtilgjengelig.v1";
const Ctx = createContext<HengerUtilgjengeligStoreValue | null>(null);
...
function normalizeLoaded(data: unknown): HengerUtilgjengelig[] {
  ...
  return data
    ...
    .map((x) => {
      ...
      const hengerId = String(x.hengerId ?? "");
      ...
      if (!id || !hengerId || !fraDato || !tilDato) return null;
      return { id, hengerId, type, fraDato, tilDato, planlagt, kommentar } as HengerUtilgjengelig;
    })
    .filter(Boolean) as HengerUtilgjengelig[];
}
